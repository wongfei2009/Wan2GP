...
Module names are identical to the original checkpoint. The legacy `weight_g` / `weight_v` checkpoint
keys are translated at load time to PyTorch's current weight-norm parametrization spelling. Registered
Kaiser-window resampling `filter` buffers of the anti-aliased activations remain unchanged.
"""
...
class MiniMaxH3AudioAttnProcessor:
    ...
    def __call__(self, attn: "MiniMaxH3AudioCausalAttention", hidden_states: list[torch.Tensor]) -> torch.Tensor:
