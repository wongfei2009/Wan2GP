...
def _load_s2mel_checkpoint_state(path):
    if str(path).lower().endswith(".safetensors"):
        ...
        for key, value in flat_state.items():
            ...
            norm_key = norm_key[7:] if norm_key.startswith("models.") else norm_key
            if "." not in norm_key:
                continue
            module_name, param_name = norm_key.split(".", 1)
            nested_net.setdefault(module_name, {})[param_name] = value
        return {"net": nested_net, "epoch": 0, "iters": 0}
...
def load_checkpoint(
    ...
    state = _load_s2mel_checkpoint_state(path)
    params = state["net"]
