...
def _reduce_kv_kernel(
    ...
    dims = tl.arange(0, D)
    valid = tokens < T
    k_offsets = batch * SKB + tokens[:, None] * SKT + head * SKH + dims[None, :]
    v_offsets = batch * SVB + tokens[:, None] * SVT + head * SVH + dims[None, :]
    k_values = tl.load(k + k_offsets, mask=valid[:, None], other=0.0)
...
def _diag_threshold_kernel(
    ...
    dims = tl.arange(0, D)
    valid = tokens < T
    offsets = batch * SQB + tokens[:, None] * SQT + head * SQH + dims[None, :]
