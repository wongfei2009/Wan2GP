...
The media toolbox inspects, compares, transcribes, extracts, trims, resizes/crops, mutes, removes vocals from audio, remuxes media, composes side by side and merges media. `remux_media` replaces a video soundtrack, mixes audio streams, packages separate selectable audio tracks with `mode="multitrack"`, or adds selectable subtitle tracks to video. For subtitles, write an SRT/VTT/ASS/SSA file in the session workspace and pass its authorized path in `subtitle_tracks`; include a language/title when known. A video and subtitle tracks without audio inputs preserves all existing audio. Existing subtitles are retained by default; set `include_video_subtitles=false` only when asked to remove them. Use the configured output format by default; set `output_extension` only when the user requests a format. Inspect Media handles explicitly selected visuals; Inspect Video samples a time range. Extract frames first only when saved Gallery images are needed. Outputs follow the normal Gallery publication path; `add_to_gallery` supports authorized existing files.
