...
def default_config() -> dict[str, Any]:
    return {APPRISE_URLS_KEY: [], SECURE_STORAGE_KEY: True, CREDENTIAL_ID_KEY: "", NOTIFY_GENERATION_KEY: False, NOTIFY_QUEUE_COMPLETE_KEY: False, NOTIFY_QUEUE_INTERRUPTED_KEY: False}
...
def apply_defaults(config: dict[str, Any]) -> None:
    ...
    config.setdefault(CREDENTIAL_ID_KEY, "")
...
def cleanup_config_update(old_config: dict[str, Any], new_config: dict[str, Any]) -> None:
    ...
    if old_config.get(SECURE_STORAGE_KEY, False) and credential_id and credential_id != new_config[CREDENTIAL_ID_KEY]:
        if not secure_store.availability_error():
